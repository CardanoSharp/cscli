...
public class CommandParserShould
{
    ...
    "addr_test")]
    public void ParseArgs_Correctly_To_Bech32EncodeCommand_When_Options_Are_Valid(string args, string expectedValue, string expectedPrefix)
    {
        var command = CommandParser.ParseArgsToCommand(args.Split(' '));

        var bech32DecodeCommand = (Bech32EncodeCommand)command;
        bech32DecodeCommand.Should().BeOfType<Bech32EncodeCommand>();
    ...
    public void ParseArgs_Correctly_To_Bech32DecodeCommand_When_Options_Are_Valid(string args, string expectedValue)
    {
        var command = CommandParser.ParseArgsToCommand(args.Split(' '));

        var bech32DecodeCommand = (Bech32DecodeCommand)command;
        bech32DecodeCommand.Should().BeOfType<Bech32DecodeCommand>();
        bech32DecodeCommand.Value.Should().Be(expectedValue);
    }
