...
public class BuildSimplePaymentTransactionCommand : ICommand
{
    ...
    public async ValueTask<CommandResult> ExecuteAsync(CancellationToken ct)
    {
        ...
        var sourceAddressInfo = (await addressClient.GetAddressInformation(FromAddress)).Content;
        ...
        var txOutput = new PendingTransactionOutput(ToAddress, new AggregateValue(Lovelaces, Array.Empty<NativeAssetValue>()));
        ...
        txBodyBuilder.AddOutput(new Address(FromAddress), txChangeOutput.Lovelaces);
        // Witnesses
        var paymentSkey = GetPrivateKeyFromBech32SigningKey(FromAddressSigningKey);
